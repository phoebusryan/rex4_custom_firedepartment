<?php
	...
	switch ($func) {
		case '':
			$list = rex_list::factory('SELECT `id`, `start_date` FROM `'.$REX['TABLE_PREFIX'].'firedepartment_operations` ORDER BY `end_date` DESC');
			...
			$list->setColumnLabel('start_date', 'Einsatzbeginn'); //todo: Translate
		...
		case 'edit':
			...
			//Start - field for alert_id
				...
				$field->setLabel('Alert ID'); //todo: translate
			...
			//Start - field for unit_id
				$field = &$form->addSelectField('config_unit_ids');
				$field->setLabel('Unit ID'); //todo: translate
			...
			$field->setLabel('Kurzbericht');//todo: translate
			
			$field = &$form->addTextareaField('report_long', null, ['rows' => 6]);
			$field->setLabel('Bericht');//todo: translate
			
			$field = &$form->addTextField('place');
			$field->setLabel('Einsatzort');//todo: translate
			
			$field = &$form->addTextField('start_date', null, ['class' => 'date']);
			$field->setLabel('Alarmierung');//todo: translate
			
			$field = &$form->addTextField('end_date', null, ['class' => 'date']);
			$field->setLabel('Einsatzende');//todo: translate
